...
<head>
    ...
    <title>OG Tech — Add New Product</title>
...
<div class="flex min-h-screen">
    ...
    <main class="flex-1">
        ...
        <!-- TOP NAVBAR -->
        <header class="bg-transparent px-8 py-5 flex justify-between items-center">
            ...
            <div class="flex items-center gap-4">
                <input
                    type="text"
                    placeholder="Search"
                    class="w-56 rounded-full px-4 py-2 text-sm border">

                <div class="w-9 h-9 rounded-full bg-blue-950 text-white flex items-center justify-center text-xs font-semibold">
                    DC
                </div>
            </div>
        ...
        <form id="addProductForm" class="px-8 pb-10">
            @csrf
...
<script>
    // Live preview of the chosen filename in the upload box
    document.getElementById('productImage').addEventListener('change', function(){
        const label = document.getElementById('uploadLabel');
        label.textContent = this.files.length ? this.files[0].name : 'Upload Product Image';
    });

    let saveMode = 'publish';

    function saveProduct(mode){
        saveMode = mode;
        document.getElementById('addProductForm').requestSubmit();
    }
    ...
    document.getElementById('addProductForm').addEventListener('submit', function(e){
        ...
        .then(data => {
            if(data.success){
                alert(saveMode === 'draft' ? 'Product saved as draft.' : 'Product saved successfully!');
